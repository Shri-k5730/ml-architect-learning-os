...
def normalize_rewards_state(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for row in state.get("history", []):
        ...
        clear = _is_clear(decision=decision, status=status, completed=row.get("completed") if "completed" in row else None)
        row["completed"] = clear
    ...
    if completed_topics:
        _award_badge(state, "first_clear", "First Clear", "Completed the first level.", awarded_dummy)

    if best_streak >= 3:
        _award_badge(state, "clear_streak_3", "Clear Streak x3", "Completed 3 levels in a row.", awarded_dummy)

    return state


def _load_local_rewards_state() -> Dict[str, Any]:
...
def load_rewards_state(prefer_cloud: bool = True) -> Dict[str, Any]:
    if prefer_cloud:
        try:
            from src.utils.supabase_store import fetch_state

            cloud_state = fetch_state("rewards_state")
            if isinstance(cloud_state, dict):
                state = normalize_rewards_state(cloud_state)
                _save_local_rewards_state(state)
                return state
        except Exception:
            pass

    state = normalize_rewards_state(_load_local_rewards_state())
    _save_local_rewards_state(state)
    return state


def _save_local_rewards_state(state: Dict[str, Any]) -> None:
    REWARDS_STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
    REWARDS_STATE_PATH.write_text(json.dumps(state, indent=2, ensure_ascii=False), encoding="utf-8")


def save_rewards_state(state: Dict[str, Any]) -> None:
    normalized = normalize_rewards_state(state)
    _save_local_rewards_state(normalized)
    try:
        from src.utils.cloud_state import persist_rewards_to_supabase
...
def apply_evaluation_rewards(
    ...
    state = load_rewards_state(prefer_cloud=True)
    ...
    if clear and not state.get("history"):
        _award_badge(state, "first_clear", "First Clear", "Completed the first level.", badges_awarded)
    if stars >= 3:
        _award_badge(state, "three_star_scholar", "Three-Star Scholar", "Earned at least 3 stars on a level.", badges_awarded)
    if stars >= 4:
        _award_badge(state, "four_star_architect", "Four-Star Architect", "Earned at least 4 stars on a level.", badges_awarded)
    if stars == 5:
        _award_badge(state, "five_star_master", "Five-Star Master", "Earned 5 stars on a level.", badges_awarded)
    if _as_int(scores.get("architect_reasoning"), 0) >= 4:
        _award_badge(state, "architect_eye", "Architect Eye", "Reached strong architect reasoning on a lesson.", badges_awarded)
    if _as_int(state["streaks"].get("current_completion_streak"), 0) >= 3:
        _award_badge(state, "clear_streak_3", "Clear Streak x3", "Completed 3 levels in a row.", badges_awarded)
    ...
    summary = {
        ...
        "status": "completed" if clear else decision_norm,
        "completed": clear,
        "xp_earned": xp_earned,
