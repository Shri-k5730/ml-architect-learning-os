...
def select_active_topic(progress_rows: List[Dict[str, Any]], latest_evaluation: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    if latest_evaluation:
        ...
        if topic_id and (status in REPAIR_STATUSES or next_action == "retry_same_topic"):
            # A failed redo should stay active only if the topic has not already
            # reached mastery in a previous/best attempt. Best mastery beats
            # latest failed redo for progression.
            topic_row = next((r for r in progress_rows if str(r.get("topic_id")) == str(topic_id)), None)
            if topic_row is None or not is_mastered(topic_row):
                return str(topic_id)
