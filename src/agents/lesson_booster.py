...
ADVANCED_ML_BOOSTERS: Dict[str, Dict[str, Any]] = {
    ...
    "mlf_019": {
        "plain_language": "Interpretability shows what the model leaned on. It does not prove what caused the real-world outcome. Treat the explanation as an investigation clue.",
        "worked_example": "If humidity is the top feature for defects, conclude that the model relied on humidity. Do not conclude that humidity caused defects. Check correlated factors like shift, season, supplier batch, machine setting, and sensor drift before any process action.",
        "production_trap": "Stakeholders treat SHAP-style explanations as causal proof and change the process without domain review, experiment, or approval path.",
        "mission_hint": "Use the clue-not-proof frame: what the explanation shows, what it cannot prove, what to validate, and what governance blocks unsafe action.",
        "key_distinctions": [
            "Global explanation describes average model behavior; local explanation describes one prediction.",
            "Feature importance is not causal proof; it only reports model reliance or contribution under the explanation method.",
            "Correlated features can split, swap, or distort importance, so the ranking may be unstable.",
            "Architects must define explanation usage limits, review workflow, audit trail, and approval path before business action.",
        ],
        "answer_frame": [
            ...
            "4. Define validation: domain review, data slice, correlation check, experiment, or process evidence.",
            "5. Add governance: audit trail, explanation caveat, global/local separation, and approval path.",
        ],
