...
SCAFFOLD_PHRASES = [
    ...
    "this is no longer a lesson answer",
]


def _scaffold_findings(answer: str) -> List[Dict[str, str]]:
    answer_l = _normalize(answer)
    for phrase in SCAFFOLD_PHRASES:
        if phrase in answer_l:
            return [{
                "evidence": phrase,
                "issue": "This looks like tutor/scaffold guidance copied into the answer rather than a direct response to the question.",
                "correction": "Rewrite it as your own architecture decision: name the scenario, evidence, decision, owner and action.",
                "severity": "blocking",
            }]
    return []
...
def verify_draft_answers(
    ...
    for question in assessment.questions:
        ...
        concept_findings = [*_profile_gap(concept_note.topic_id, answer), *_contract_findings(concept_note.topic_id, question.question_id, answer), *_scaffold_findings(answer)]
