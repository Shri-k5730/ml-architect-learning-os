...
ADVANCED_ML_BLUEPRINTS: Dict[str, Dict[str, Any]] = {
    ...
    "mlf_019": {
        ...
        "module": "Advanced ML",
        "definition": "Interpretability explains model behavior, but explanations are investigation evidence, not automatic proof of causality, correctness, or business truth.",
        "plain_intuition": "An explanation tells you what the model leaned on. It does not prove what caused the real-world outcome.",
        "why_it_exists": "Stakeholders need to understand and challenge predictions, but explanation tools are dangerous when teams treat correlation or model reliance as causation.",
        "core_mechanism": "Global explanations summarize broad model behavior. Local explanations explain one prediction. Feature importance reports model reliance under an explanation method; correlated features can split or distort importance and none of this proves causal effect.",
        "worked_example": "A defect model says humidity is important. Conclude that the model relied on humidity. Do not conclude humidity caused defects. Check correlated factors like machine setting, shift, season, supplier batch, sensor drift, and label timing before any process action.",
        "nuances": ["Global and local explanations answer different questions.", "Correlated features can split, swap, or distort importance.", "Explanation is useful for review, debugging, and governance, but not causal proof.", "A safe explanation workflow ends in validation or approved investigation, not automatic process change."],
        ...
        "common_confusions": ["Treating SHAP/feature importance as causality.", "Treating a convincing local explanation as proof the model is correct.", "Showing explanation charts without caveats or action policy.", "Ignoring instability caused by correlated features."],
